...
namespace ZYN.BLOG.BLL
{
    public abstract class BaseService<T> : IBLL.IBaseService<T> where T : class,new()
    {
        ...
        private IDAL.IDbSession _currentDbSession;
        public IDAL.IDbSession CurrentDbSession
        {
            get
            {
                if (_currentDbSession == null)
                {
                    ...
                    IDAL.IDbSessionFactory dbsessionFactory = ZYN.BLOG.Inject.SpringHelper.GetObject<IDbSessionFactory>("DbSessionFactory");
        ...
        {
            return CurrentDAL.GetDataListBy(whereLambda, orderLambda);
        } 
        #endregion
